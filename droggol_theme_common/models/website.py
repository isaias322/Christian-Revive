...
class Website(models.Model):
    ...
    def _dr_get_current_pricelist(self):
        self.ensure_one()
    ...
    def _get_current_pricelist(self):
        return self._dr_get_current_pricelist()

    def sale_get_order(self, force_create=False):
        self.ensure_one()
        if not request:
            return self.env['sale.order']
        order = getattr(request, 'cart', self.env['sale.order']) or self._get_and_cache_current_cart()
        if force_create and not order:
