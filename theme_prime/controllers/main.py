...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def _get_search_domain(self, search, category, attrib_values, search_in_description=True, search_rating=True):
        ...
        if request.httprequest.args.get('hide_out_of_stock'):
            domains = Domain.AND([domains, ['|', '|', ('is_storable', '=', False), ('allow_out_of_stock_order', '=', True), '&', ('dr_free_qty', '>', 0), ('allow_out_of_stock_order', '=', False)]])
        ...
        if ratings and search_rating:
            ...
            if rating_product_ids:
                domains = Domain.AND([domains, [('id', 'in', rating_product_ids)]])
            else:
                domains = Domain.AND([domains, [('id', 'in', [])]])
        return domains
    ...
    def _shop_get_query_url_kwargs(self, search, min_price, max_price, order=None, tags=None, **post):
        result = super()._shop_get_query_url_kwargs(search, min_price, max_price, order=order, tags=tags, **post)
        result.update(
...
class ThemePrimeMainClass(http.Controller):
    ...
    def _get_pricelist_data(self, fields=['name'], limit=20, order=None, extras={}):
        ...
        for pricelist in pricelists:
            ...
            result.append(data)
        return {'active_pricelist': request.website._get_current_pricelist().id, 'pricelists': result}
    ...
    @http.route(['/theme_prime/get_dialog_content'], type='jsonrpc', website=True, auth="public")
